...
TEMPLATE_FEATURES = {
    ...
    "template_all_atom_mask": torch.float64,
    "template_all_atom_positions": torch.float64,
    "template_domain_names": str,
    "template_sequence": str,
    "template_sum_probs": torch.float64,
}
...
def _check_residue_distances(
    ...
    all_positions_mask: torch.Tensor,
    ...
    for i, (coords, mask) in enumerate(zip(all_positions, all_positions_mask)):
        ...
        if this_is_unmasked:
            ...
            if prev_is_unmasked:
                distance = torch.linalg.norm(this_calpha - prev_calpha)
...
def _get_atom_positions(
    ...
    all_atom_positions = torch.Tensor(all_atom_positions).type(torch.float64)
    all_atom_mask = torch.Tensor(all_atom_mask).type(torch.float64)
...
def _find_template_in_pdb(
    ...
    for chain_id, chain_sequence in mmcif_object.chain_to_seqres.items():
        if chain_sequence and (template_sequence in chain_sequence):
            logging.getLogger(__name__).info("Found a sequence-only match %s_%s.", pdb_id, chain_id)
            mapping_offset = chain_sequence.find(template_sequence)
...
def empty_template_feats(n_res) -> FeatureDict:
    return {
        "template_aatype": torch.zeros((0, n_res, 22),dtype=torch.float64),
        "template_all_atom_positions": 
            torch.zeros((0, n_res, 37, 3),dtype=torch.float64),
        "template_sum_probs": torch.zeros((0, 1),dtype=torch.float64),
        "template_all_atom_mask": torch.zeros((0, n_res, 37),dtype=torch.float64)
    }


def parse_template_hits(
    alignment_dir: str,
...
def make_template_features(
    ...
    if(len(hits_cat) == 0 or template_featurizer is None):
        template_features = empty_template_feats(len(itorchut_sequence))
    else:
        templates_result = template_featurizer.get_templates(
            query_sequence=itorchut_sequence,
...
def _extract_template_features(
    ...
    try:
        ...
        logging.getLogger(__name__).warning(__name__)
    ...
    all_atom_positions = torch.split(
        all_atom_positions, 1,dim=0
    )

    all_atom_masks = torch.split(all_atom_mask, 1,dim=0)
    ...
    for _ in query_sequence:
        ...
        templates_all_atom_positions.append(
            torch.zeros((residue_constants.atom_type_num, 3))
        )
        templates_all_atom_masks.append(
            torch.zeros(residue_constants.atom_type_num)
        )
        output_templates_sequence.append("-")
    templates_all_atom_positions = torch.stack(templates_all_atom_positions,dim=0)
    templates_all_atom_masks = torch.stack(templates_all_atom_masks,dim=0)
    ...
    if torch.sum(templates_all_atom_masks) < 5:
    ...
    return (
        {
            ...
            "template_all_atom_mask": torch.Tensor(templates_all_atom_masks),
            "template_sequence": output_templates_sequence,
            "template_aatype": torch.Tensor(templates_aatype),
...
def _process_single_hit(
    ...
    if not os.path.exists(cif_path):
        return SingleHitResult(features=None, error=None, warning=f"template {cif_path} do not exist")
    logging.getLogger(__name__).info(
    ...
    try:
        ...
        features["template_sum_probs"] = torch.Tensor([hit.sum_probs])
...
def _prefilter_hit(
    ...
    try:
        ...
        logging.getLogger(__name__).info("%s: %s", query_pdb_code, msg)
...
def _parse_release_dates(path: str) -> Mapping[str, datetime.datetime]:
    ...
    return {
        row["pdb_name"].upper(): to_date(row["release_date"])
...
class TemplateHitFeaturizer:
    ...
    def __init__(
        ...
        if not glob.glob(os.path.join(self._mmcif_dir, "*.cif")) and not glob.glob(os.path.join(self._mmcif_dir,"*", "*.cif")):
            logging.getLogger(__name__).error("Could not find CIFs in %s", self._mmcif_dir)
    ...
    def get_templates(
        ...
        logging.getLogger(__name__).info("Searching for template for: %s", query_pdb_code)
        ...
        if query_release_date:
            if not isinstance(query_release_date,datetime.datetime):
                query_release_date = datetime.datetime.strptime(
                    query_release_date, "%Y-%m-%d"
                )
        ...
        if(self._shuffle_top_k_prefiltered):
            ...
            perm = torch.randperm(stk)
            idx[:stk].scatter_(0, perm, idx[:stk])
        ...
        for name in template_features :
            if not isinstance(TEMPLATE_FEATURES[name],torch.dtype):
                continue
            if num_hits > 0:
                template_features[name] = torch.stack(
                    template_features[name], dim=0
                ).type(TEMPLATE_FEATURES[name])
            else:
                # Make sure the feature has correct dtype even if empty.
                template_features[name] = torch.Tensor(
                    [],
                ).type(TEMPLATE_FEATURES[name])
